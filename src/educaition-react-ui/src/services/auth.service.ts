...
export const AuthService = createApi({
  ...
  endpoints: (builder) => ({
    authenticate: builder.query<
      User,
      HttpBaseRequest<{ id: string; token: string }>
    >({
      query: ({ id, token }) => ({
    ...
    login: builder.mutation<
      User,
      HttpBaseRequest<{ email: string; password: string }>
    >({
    ...
    updateAuthUser: builder.mutation<
      User,
      HttpBaseRequest<{ id: number; language: string }>
    >({
      query: (user) => ({
    ...
    uploadProfilePicture: builder.mutation<
      User,
      HttpBaseRequest<{ userId: string; formData: FormData }>
    >({
    ...
    deleteProfilePicture: builder.query<
      User,
      HttpBaseRequest<{ userId: string }>
    >({
      query: ({ userId }) => ({
